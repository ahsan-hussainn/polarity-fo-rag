# Finding: principal-extraction benchmark (proxy labels) + email grading

**Date:** 2026-07-12 (measured pre-curation) · **Data:** silver = 59 firms, 425 people (251
model-flagged principals). **Correction note (2026-07-20):** this measures the extractor against a
**title rubric**, i.e. a *proxy-label benchmark* (422/425 labels generated by rule, 3 manual
overrides) — not independent human ground truth about real investment authority. The wording below
has been narrowed accordingly; ADR-0021's Schedule A pass is what actually establishes authority.
The numbers are kept as-measured; the emails were later re-adjudicated per proven decision-maker
(WS3, `docs/findings/decision-maker-evidence.md`).

Two things are graded here: the extractor's **principal vs. staff** call against the rubric, and the
**email** contact cell. The point is not high numbers — it is honest ones, method exposed so a reader
can re-run them (`python -m pipeline.cli gt-score` / `gt-crosscheck`).
...
- **The proxy labels** are generated from each person's title alone, **blind to the model's
  prediction** (`pipeline/eval.py::adjudicate_title`), by a documented rubric: a principal has capital-allocation
...
Schedule A (named owners/officers) replaces the title rubric with a registry — the strongest next
