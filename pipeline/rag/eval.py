...
CASES = [
    ...
    {"name": "out of scope, token-colliding with a firm (Zurich -> Marcuard)",
     "q": "What is the weather in Zurich today?",
     "want": lambda d: not d["sources"],
     "why": "shares a token with a Zurich firm but is not about the dataset; the cosine relevance "
            "floor (ADR-0026) must refuse it -- the case token-matching could not catch"},
    {"name": "out of scope, general knowledge (no token collision)",
     "q": "What is the capital of France?",
     "want": lambda d: not d["sources"],
     "why": "a query unrelated to the dataset must be refused, not answered with the nearest firms"},
    {"name": "out of scope, unrelated count (aggregate-path hole)",
     "q": "How many countries are there in Africa?",
     "want": lambda d: not d["sources"],
     "why": "an out-of-scope COUNT must not be answered off the dataset total via the aggregate path"},
    {"name": "in scope, restrictive filter must NOT be refused (floor false-positive guard)",
     "q": "Which family offices are in Ohio?",
     "want": lambda d: d["verification"]["passed"],
     "why": "topical query with a narrow state filter stays in scope (floor is unfiltered) -- guards "
            "against the relevance floor wrongly refusing real dataset questions"},
